...
public:
	...
	void update(const unsigned char* buf, size_type length){// compute number of bytes mod 64
	...
	void update(const char* buf, size_type length) { update((const unsigned char*)buf, length); }
	MD5& finalize(){
	...
	friend std::ostream& operator<<(std::ostream&, MD5 md5) { return std::cout << md5.hexdigest(); }
